...
public class AdminUsersController : ControllerBase
{
    ...
    /// <summary>
    /// API cấp quyền cụ thể cho một Admin hoặc Moderator khác (Yêu cầu SuperAdmin).
    /// </summary>
    [RequirePermission("permission.grant")]
    [HttpPost("{targetUserId:guid}/permissions")]
    public async Task<IActionResult> GrantPermission(Guid targetUserId, [FromBody] UpdatePermissionRequest request)
    {
        try
        {
            var adminIdString = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            await _adminUserService.GrantPermissionAsync(adminId, targetUserId, request.PermissionCode);
            return Ok(new { Message = $"Đã cấp quyền '{request.PermissionCode}' thành công." });
        }
        catch (Exception ex)
        {
            return BadRequest(ex.Message);
    ...
    /// <summary>
    /// API thu hồi quyền cụ thể của một Admin hoặc Moderator khác (Yêu cầu SuperAdmin).
    /// </summary>
    [RequirePermission("permission.grant")]
    [HttpDelete("{targetUserId:guid}/permissions")]
    public async Task<IActionResult> RevokePermission(Guid targetUserId, [FromQuery] string permissionCode)
    {
        try
        {
            var adminIdString = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            await _adminUserService.RevokePermissionAsync(adminId, targetUserId, permissionCode);
            return Ok(new { Message = $"Đã thu hồi quyền '{permissionCode}' thành công." });
        }
        catch (Exception ex)
        {
            return BadRequest(ex.Message);
    ...
    /// <summary>
    /// API khôi phục danh sách quyền của người dùng (Admin/Moderator) về mặc định theo vai trò (Yêu cầu SuperAdmin).
    /// </summary>
    [RequirePermission("permission.grant")]
    [HttpPost("{targetUserId:guid}/permissions/reset")]
    public async Task<IActionResult> ResetPermissions(Guid targetUserId)
    {
        try
        {
            var adminIdString = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            await _adminUserService.ResetPermissionsToDefaultAsync(adminId, targetUserId);
            return Ok(new { Message = "Đã khôi phục các quyền của người dùng về mặc định thành công." });
        }
        catch (Exception ex)
        {
            return BadRequest(ex.Message);
